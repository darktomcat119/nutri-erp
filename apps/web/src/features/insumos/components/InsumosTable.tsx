...
import { Pencil, Trash2, Plus, Search, Download, Upload, UtensilsCrossed } from 'lucide-react';
...
export function InsumosTable(): JSX.Element {
  ...
  const openCreate = async (): Promise<void> => {
    ...
    reset({
      ...
      unidad: 'kg',
      presentacion: '',
      cantidadPorDisplay: '' as unknown as number,
      costoUnitario: 0,
      proveedorId: '',
      origen: '',
    });
  ...
  const onSubmit = async (fd: FormData): Promise<void> => {
    try {
      const payload = {
        ...
        cantidadPorDisplay:
          fd.cantidadPorDisplay === '' || fd.cantidadPorDisplay === undefined
            ? null
            : fd.cantidadPorDisplay,
      };
      if (editing) {
        await api.patch(`/insumos/${editing.id}`, payload);
  ...
  const handleDelete = async (): Promise<void> => {
    if (!deleteId) return;
    try {
      await api.delete(`/insumos/${deleteId}`);
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>): Promise<void> => {
    ...
    try {
      ...
      const res = await api.post('/insumos/import-excel-preview', formData, {
  ...
  const handleApplyPreview = async (): Promise<void> => {
    ...
    try {
      ...
      const res = await api.post('/insumos/import-excel-stream', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
  ...
  const columns: ColumnDef<Insumo>[] = [
    ...
    {
      accessorKey: 'unidad',
      header: ({ column }) => <SortableHeader column={column}>Unidad</SortableHeader>,
    },
    {
    ...
    {
      accessorKey: 'cantidadPorDisplay',
      header: 'Cant/Display',
      cell: ({ row }) => row.original.cantidadPorDisplay ?? '—',
    },
    {
      id: 'proveedor',
      header: 'Proveedor',
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (!origen) return '—';
        return <Badge variant={origen === 'Compras' ? 'default' : 'secondary'}>{origen}</Badge>;
      },
    },
    {
      id: 'actions',
  ...
  return (
    <div>
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button onClick={openCreate} className="w-full sm:w-auto">
                <Plus className="h-4 w-4 mr-2" /> Nuevo Insumo
              </Button>
            </DialogTrigger>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={watch('presentacion') || ''}
                      onValueChange={(v) => setValue('presentacion', v)}
                    >
                      <SelectTrigger className="min-h-[44px]">
                        <SelectValue placeholder="Seleccionar" />
                      </SelectTrigger>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      min={0}
                      step={1}
                      {...register('cantidadPorDisplay', {
                        setValueAs: (v: string) => (v === '' ? '' : parseInt(v, 10)),
                      })}
                      placeholder="0"
                    />
                    {errors.cantidadPorDisplay && (
                      <p className="text-xs text-red-600">{errors.cantidadPorDisplay.message}</p>
                    )}
                  </div>
                  <div className="space-y-2">
                    <Label>
                    ...
                    <Select
                      value={watch('origen') || ''}
                      onValueChange={(v) => setValue('origen', v)}
                    >
                      <SelectTrigger className="min-h-[44px]">
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="p-0">
                  <EmptyState
                    ...
                    description="Crea un insumo con el boton '+ Nuevo Insumo'."
                  />
                </TableCell>
              </TableRow>
            )}
      ...
      <ImportPreviewDialog
        ...
        title="Importar insumos desde Excel"
        preview={previewData}
      ...
      <ConflictResolutionDialog
        ...
        title="Revisar actualizaciones de insumos"
      ...
      <ProgressDialog
        ...
        title="Importando insumos desde Excel"
