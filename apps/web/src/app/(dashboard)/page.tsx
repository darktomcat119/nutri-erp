...
const quickActions = [
  {
    ...
    description: 'Crear pedido semanal',
    href: '/mi-requisicion',
    icon: FileText,
  ...
  {
    label: 'Requisiciones',
    description: 'Revisar y aprobar pedidos',
    href: '/requisiciones',
    icon: FileText,
    color: 'from-blue-500 to-blue-600',
  ...
  },
  {
    ...
    description: 'Generar y gestionar OC',
    href: '/ordenes-compra',
    icon: ShoppingCart,
  ...
  {
    label: 'Ruta del Dia',
    description: 'Ver productos a comprar',
    href: '/ruta',
    icon: Truck,
  ...
  {
    label: 'Entregas',
    description: 'Generar ordenes de entrega',
    href: '/entregas',
    icon: Package,
  ...
  },
  {
    ...
    roles: ['ADMIN', 'SUPERVISOR', 'ENCARGADO'],
  },
  {
    label: 'Control Financiero',
    description: 'Presupuesto vs gasto real',
    href: '/financiero',
    icon: DollarSign,
  ...
  {
    label: 'Reportes',
    description: 'Resumen semanal completo',
    href: '/reportes',
    icon: BarChart3,
  ...
  return (
    <Link href={href}>
      <Card className="group relative overflow-hidden border-0 shadow-sm hover:shadow-lg transition-all duration-300 cursor-pointer h-full">
        <div
          className={`absolute top-0 right-0 w-24 h-24 rounded-full ${color} opacity-[0.04] -translate-y-6 translate-x-6 group-hover:scale-150 transition-transform duration-500`}
        />
        <CardContent className="pt-4 pb-3 px-4 sm:pt-5 sm:pb-4 sm:px-5 relative">
          <div className="flex items-start justify-between mb-3">
            <div
              className={`h-10 w-10 rounded-xl ${color} bg-opacity-10 flex items-center justify-center`}
            ...
            <div className="flex items-center gap-1.5">
              ...
              {trend && (
                <span
                  className={`flex items-center gap-0.5 text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${trend.up ? 'text-emerald-700 bg-emerald-50' : 'text-red-600 bg-red-50'}`}
                >
                  {trend.up ? (
    ...
    <div className="space-y-1.5">
      ...
      <div className="flex items-center justify-between">
        <span className={`text-[10px] ${isOver ? 'text-red-500 font-semibold' : 'text-slate-400'}`}>
          {isOver
            ? `Excedido ${formatCurrencyFull(spent - budget)}`
            : `Disponible ${formatCurrencyFull(budget - spent)}`}
        </span>
...
export default function DashboardPage(): JSX.Element {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      ...
      const safe = (p: Promise<{ data: unknown }>) =>
        p
          .then((r) => {
      ...
      // Admin-only endpoints are skipped for non-admin users to avoid harmless
      // 403s cluttering the browser console (and wasted network calls).
      const adminOnly = <T,>(url: string): Promise<T | null> =>
        isAdmin ? (safe(api.get(url)) as Promise<T | null>) : Promise.resolve(null);

      const [reqs, ocs, entregas, receps, prods, provs, usrs, gastos, sucursales, resumen] =
        await Promise.all([
          ...
          adminOnly('/usuarios'),
          adminOnly(`/reportes/gastos-proveedor/${semana}`),
          safe(api.get('/sucursales')),
          adminOnly(`/reportes/resumen-semanal/${semana}`),
        ]);

      const reqArr = (reqs || []) as Array<{
        estado: string;
        createdAt: string;
        sucursal: { codigo: string };
        creadoPor: { nombre: string };
      }>;
      const ocArr = (ocs || []) as Array<{
        estado: string;
        folio: string;
        semana: string;
        createdAt: string;
      ...
      const gastosRaw = (gastos || []) as Array<{
      ...
      const sucArr = (sucursales || []) as Array<{
        id: string;
        codigo: string;
        nombre: string;
        activa?: boolean;
      ...
      const budgetResults = await Promise.all(
        activeBranches.map(async (suc): Promise<BranchBudget> => {
          try {
            ...
            const pres = (d as Record<string, unknown>).presupuesto as
              | Record<string, unknown>
              | undefined;
            const gasto = (d as Record<string, unknown>).gastoReal as
      ...
      setSupplierData(
        gastosArr.slice(0, 8).map((g) => ({
          name: g.proveedor?.length > 12 ? g.proveedor.slice(0, 12) + '…' : g.proveedor,
          total: g.total,
        })),
      );
      ...
      setTopProducts(
        sorted.slice(0, 5).map((p) => ({ name: p.nombre, cost: Number(p.costoDisplay), qty: 0 })),
      );
      ...
      for (const r of reqArr.slice(0, 4)) {
        acts.push({
          ...
          label: `Requisicion ${r.sucursal?.codigo}`,
          detail: r.estado,
          time: r.createdAt,
        });
      }
      ...
      for (const rec of recArr.slice(0, 3)) {
        acts.push({
          ...
          label: `Recepcion ${rec.sucursal?.codigo}`,
          detail: `por ${rec.recibidoPor?.nombre}`,
          time: rec.createdAt,
        });
      }
  ...
  const radialData = [
    {
      ...
      fill: budgetPct > 100 ? '#ef4444' : budgetPct >= 80 ? '#f59e0b' : '#10b981',
    },
  ];
  ...
  return (
    <div className="space-y-5">
        ...
        roleLabel={
          isAdmin
            ...
            : isSupervisor
              ...
              : isEncargado
                ? user?.sucursal?.nombre || 'Mi Sucursal'
                : 'Ruta de Compras'
        }
        semana={semana}
      />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard
          label="Requisiciones"
          value={String(stats.reqPendientes)}
        ...
        <StatCard
          label="Ordenes de Compra"
          value={String(stats.ocTotal)}
          subtitle={`${stats.ocCompletadas} completadas`}
          icon={ShoppingCart}
          href="/ordenes-compra"
          color="bg-violet-500"
          trend={
            stats.ocCompletadas > 0 ? { value: `${stats.ocCompletadas}`, up: true } : undefined
          }
        />
        <StatCard
          label="Entregas"
        ...
        <StatCard
          ...
          subtitle={
            totalBudget > 0 ? `${budgetPct.toFixed(0)}% del presupuesto` : 'Sin presupuesto'
          }
          icon={DollarSign}
          href="/financiero"
          color="bg-amber-500"
          trend={
            stats.gastoTotal > 0
              ? { value: formatCurrency(stats.gastoTotal), up: budgetPct <= 100 }
              : undefined
          }
        />
      </div>
              ...
              <div className="flex items-center justify-center">
                <div className="relative h-[140px] w-[140px]">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center pt-2">
                    <span
                      className={`text-2xl font-bold ${budgetPct > 100 ? 'text-red-600' : budgetPct >= 80 ? 'text-amber-600' : 'text-emerald-600'}`}
                    >
                      {budgetPct.toFixed(0)}%
                    </span>
              ...
              <div className="space-y-3 pt-2 border-t">
                ...
                {branchBudgets.map((b) => (
                  <div key={b.id} className="space-y-2">
                    <BudgetGauge
                      label={`${b.codigo} — Mostrador`}
                      budget={b.budgetMos}
                      spent={b.spentMos}
                      color="bg-blue-500"
                    />
                    <BudgetGauge
                      label={`${b.codigo} — Insumos`}
                      budget={b.budgetIns}
                      spent={b.spentIns}
                      color="bg-emerald-500"
                    />
                  ...
                  <>
                    ...
                    <div className="flex justify-center gap-4 text-[11px]">
                      <span className="flex items-center gap-1.5">
                        ...
                        MOS {formatCurrencyFull(stats.gastoMos)}
                      </span>
                      <span className="flex items-center gap-1.5">
                        <span className="h-2 w-2 rounded-full bg-emerald-500" />
                        INS {formatCurrencyFull(stats.gastoIns)}
                      </span>
                    </div>
                  </>
                ) : (
                  <div className="h-[130px] flex items-center justify-center text-xs text-slate-300">
                    Sin datos
                  </div>
                )}
              </CardContent>
          ...
          <Card className="border-0 shadow-sm">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-semibold text-slate-700">
                Mi Sucursal — Presupuesto
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              {(() => {
                const mine =
                  branchBudgets.find((b) => b.id === user?.sucursal?.id) || branchBudgets[0];
                return (
                  <>
                    <BudgetGauge
                      label="Mostrador (MOS)"
                      budget={mine?.budgetMos || 0}
                      spent={mine?.spentMos || 0}
                      color="bg-blue-500"
                    />
                    <BudgetGauge
                      label="Insumos (INS)"
                      budget={mine?.budgetIns || 0}
                      spent={mine?.spentIns || 0}
                      color="bg-emerald-500"
                    />
                  </>
                );
          ...
          <Card className="border-0 shadow-sm">
            ...
            <CardContent className="space-y-2">
              <div className="flex items-center gap-3 py-2">
                <CheckCircle2
                  className={`h-5 w-5 ${stats.reqTotal > 0 ? 'text-emerald-500' : 'text-slate-300'}`}
                />
                <span className="text-sm text-slate-700">Requisicion enviada</span>
              </div>
              <div className="flex items-center gap-3 py-2">
                <CheckCircle2
                  className={`h-5 w-5 ${stats.entregas > 0 ? 'text-emerald-500' : 'text-slate-300'}`}
                />
                <span className="text-sm text-slate-700">Entrega asignada</span>
              </div>
              <div className="flex items-center gap-3 py-2">
                <CheckCircle2
                  className={`h-5 w-5 ${stats.recepciones > 0 ? 'text-emerald-500' : 'text-slate-300'}`}
                />
        ...
        <Card className="border-0 shadow-sm">
          <CardContent className="py-8 text-center">
            ...
            <p className="text-sm text-slate-500 mt-1 mb-4">
              {stats.ocActivas > 0
                ? `Tienes ${stats.ocActivas} orden(es) de compra activa(s)`
                : 'No hay ordenes de compra en ejecucion'}
            </p>
            <Link href="/ruta">
              ...
              .map((action) => (
                <Link key={action.href} href={action.href}>
                  <div className="group flex items-center gap-3 p-3 rounded-xl bg-white border border-slate-100 hover:border-slate-200 hover:shadow-sm transition-all cursor-pointer">
                    <div
                      className={`h-9 w-9 rounded-lg bg-gradient-to-br ${action.color} flex items-center justify-center shrink-0 shadow-sm`}
                    >
                ...
                <div className="divide-y divide-slate-50">
                  {activities.map((act) => (
                    <div key={act.id} className="flex items-center gap-3 py-2.5">
                      <div
                        className={`h-7 w-7 rounded-full flex items-center justify-center shrink-0 ${act.color}`}
                      >
